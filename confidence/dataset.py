...
from utils.diskmap_utils import ObjListXFile, XFileDataset
...
def get_cache_path(args, split):
    ...
    cache_path = os.path.join(cache_path,
                              f'limit{args.limit_complexes}_INDEX{os.path.splitext(os.path.basename(split_path))[0]}_maxLigSize{args.max_lig_size}_H{int(not args.remove_hs)}_recRad{args.receptor_radius}_recMax{args.c_alpha_max_neighbors}'
                              + (
...
class ConfidenceDataset(Dataset):
    def __init__(self, cache_path, original_model_dir, split, device, limit_complexes,
        ...
        self.complex_graphs_cache = original_model_cache if self.use_original_model_cache else get_cache_path(args,
                                                                                                              split)
        ...
        if (not os.path.exists(
                os.path.join(self.full_cache_path, "ligand_positions.pkl")) and self.cache_creation_id is None) or \
                (not os.path.exists(os.path.join(self.full_cache_path,
                                                 f"ligand_positions_id{self.cache_creation_id}.pkl")) and self.cache_creation_id is not None):
        ...
        print(
            'Using the cached complex graphs of the original model args' if self.use_original_model_cache else 'Not using the cached complex graphs of the original model args. Instead the complex graphs are used that are at the location given by the dataset parameters given to confidence_train.py')
        ...
        if self.cache_ids_to_combine is None:
            ...
            for idx, cache_id in enumerate(self.cache_ids_to_combine):
                ...
                with open(os.path.join(self.full_cache_path, f"complex_names_in_same_order_id{cache_id}.pkl"),
                          'rb') as f:
            ...
            for idx, (rmsds_unsorted, full_ligand_positions_unsorted, names_unsorted) in enumerate(
                    zip(all_rmsds_unsorted, all_full_ligand_positions_unsorted, all_names_unsorted)):
                name_to_pos_dict = {name: (rmsd, pos) for name, rmsd, pos in
                                    zip(names_unsorted, full_ligand_positions_unsorted, rmsds_unsorted)}
        ...
        self.positions_rmsds_dict = {name: (pos, rmsd) for name, pos, rmsd in
    ...
    def get(self, idx):
        ...
        if self.balance:
            if isinstance(self.rmsd_classification_cutoff, list): raise ValueError(
            ...
            if isinstance(self.rmsd_classification_cutoff, list):
                complex_graph.y_binned = torch.tensor(
                    np.logical_and(rmsds[sample] < self.rmsd_classification_cutoff + [math.inf],
    ...
    def preprocessing(self, original_model_cache):
        ...
        print(
            'HAPPENING | loading cached complexes of the original model to create the confidence dataset RMSDs and predicted positions. Doing that from: ',
            os.path.join(self.complex_graphs_cache, "heterographs.pkl"))
        original_model_cache_path = os.path.join(original_model_cache, "heterographs.pkl")
        complex_graphs = ObjListXFile(original_model_cache_path)
        dataset = XFileDataset(complex_graphs)  # ListDataset(complex_graphs)
        ...
        for idx, orig_complex_graph in tqdm(enumerate(loader)):
            ...
            randomize_position(data_list, self.original_model_args.no_torsion, False,
            ...
            while predictions_list is None:
                try:
                    predictions_list, confidences = sampling(data_list=data_list, model=model,
                                                             ...
                                                             device=self.device, t_to_sigma=t_to_sigma,
        ...
        with open(os.path.join(self.full_cache_path,
                               f"complex_names_in_same_order{'' if self.cache_creation_id is None else '_id' + str(self.cache_creation_id)}.pkl"),
                  'wb') as f:
            pickle.dump((names), f)
