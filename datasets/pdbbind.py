...
class NoiseTransform(BaseTransform):
    ...
    def apply_noise(self, data, t_tr, t_rot, t_tor, tr_update=None, rot_update=None, torsion_updates=None):
        ...
        torsion_updates = np.random.normal(loc=0.0, scale=tor_sigma, size=data[
            'ligand'].edge_mask.sum()) if torsion_updates is None else torsion_updates
...
class PDBBind(Dataset):
    def __init__(self, root, transform=None, cache_path='data/cache', split_path='data/', limit_complexes=0,
                 ...
                 matching=True, keep_original=False, max_lig_size=None, remove_hs=False, num_conformers=1,
                 all_atoms=False,
        ...
        self.full_cache_path = os.path.join(cache_path, f'limit{self.limit_complexes}'
                                            ...
                                            + ('' if protein_path_list is None or ligand_descriptions is None else str(
            binascii.crc32(''.join(ligand_descriptions + protein_path_list).encode()))))
        ...
        self.complex_graphs = ObjListXFile(os.path.join(self.full_cache_path, "heterographs.pkl"))
        if require_ligand:
        ...
        if self.num_workers > 1:
            ...
            for i in range(len(complex_names_all) // 1000 + 1):
                ...
                if self.num_workers > 1:
                    ...
                    p.__enter__()
                with tqdm(total=len(complex_names),
                          desc=f'loading complexes {i}/{len(complex_names_all) // 1000 + 1}') as pbar:
                    map_fn = p.imap_unordered if self.num_workers > 1 else map
                    for t in map_fn(self.get_complex,
                                    zip(complex_names, lm_embeddings_chains, [None] * len(complex_names),
            ...
            fbi = open(os.path.join(self.full_cache_path, f"heterographs.pkl"), 'wb')
            fbz = open(os.path.join(self.full_cache_path, f"heterographs.pkl.sz"), 'w')
            ...
            for i in range(len(complex_names_all) // 1000 + 1):
                with open(os.path.join(self.full_cache_path, f"heterographs{i}.pkl"), 'rb') as f:
                    l = pickle.load(f)
                    for complex_graph in l:
                        complex_graph_bin = dump_to_bytes(complex_graph)
                        sz = len(complex_graph_bin)

                        write_xobject_to_bin_file(complex_graph_bin, fbi, bin_obj=True)
                        fbz.write("%s\n" % sz)
                        del complex_graph_bin
            ...
            with tqdm(total=len(complex_names_all), desc='loading complexes') as pbar:
                for t in map(self.get_complex,
                             zip(complex_names_all, lm_embeddings_chains_all, [None] * len(complex_names_all),
    ...
    def inference_preprocessing(self):
        ...
        if self.esm_embeddings_path is not None:
            ...
            lm_embeddings_chains_all = []
            if not os.path.exists(self.esm_embeddings_path): raise Exception('ESM embeddings path does not exist: ',
                                                                             self.esm_embeddings_path)
            for protein_path in self.protein_path_list:
                embeddings_paths = sorted(
                    glob.glob(os.path.join(self.esm_embeddings_path, os.path.basename(protein_path)) + '*'))
        ...
        if self.num_workers > 1:
            ...
            for i in range(len(self.protein_path_list) // 1000 + 1):
                ...
                with tqdm(total=len(protein_paths_chunk),
                          desc=f'loading complexes {i}/{len(protein_paths_chunk) // 1000 + 1}') as pbar:
                    map_fn = p.imap_unordered if self.num_workers > 1 else map
                    for t in map_fn(self.get_complex, zip(protein_paths_chunk, lm_embeddings_chains, ligands_chunk,
            ...
            with tqdm(total=len(self.protein_path_list), desc='loading complexes') as pbar:
                for t in map(self.get_complex, zip(self.protein_path_list, lm_embeddings_chains_all, ligands_list,
        ...
        for i, lig in enumerate(ligs):
            ...
            try:
                ...
                rec, rec_coords, c_alpha_coords, n_coords, c_coords, lm_embeddings = extract_receptor_structure(
                    copy.deepcopy(rec_model), lig, lm_embedding_chains=lm_embedding_chains)
                ...
                get_rec_graph(rec, rec_coords, c_alpha_coords, n_coords, c_coords, complex_graph,
...
def print_statistics(complex_graphs):
    ...
    for complex_graph in complex_graphs:
        lig_pos = complex_graph['ligand'].pos if torch.is_tensor(complex_graph['ligand'].pos) else \
...
def read_mols(pdbbind_dir, name, remove_hs=False):
    ...
    for file in os.listdir(os.path.join(pdbbind_dir, name)):
        if file.endswith(".sdf") and 'rdkit' not in file:
            ...
            if lig is None and os.path.exists(os.path.join(pdbbind_dir, name, file[
                                                                              :-4] + ".mol2")):  # read mol2 file if sdf file cannot be sanitized
                print('Using the .sdf file failed. We found a .mol2 file instead and are trying to use that.')
                lig = read_molecule(os.path.join(pdbbind_dir, name, file[:-4] + ".mol2"), remove_hs=remove_hs,
                                    sanitize=True)
            if lig is not None:
                ligs.append(lig)
    return ligs
